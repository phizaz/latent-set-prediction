...
report_options = {
    'finding_only': f'{here}/mimic_reports_only_findings_ready.csv',
...
class MimicTextCombinedDataset(BaseDataset):
    ...
    def metric(self):
        return [
            'bleu4_macro', 'bleu4_weighted', 'bleu4_chamfer_macro_f1',
            'bleu4_chamfer_macro_recall', 'bleu4_chamfer_macro_precision',
            'bleu4_chamfer_micro_f1', 'bleu4_chamfer_micro_recall',
            'bleu4_chamfer_micro_precision'
        ]

    @classmethod
    def evaluate_instance(cls,
                          pred: List[str],
                          gt: List[str],
                          mode='hungarian'):
        if mode == 'hungarian':
            scores, _, _ = best_report_bleu(gt, pred)
        elif mode == 'chamfer':
    ...
    @classmethod
    def evaluate(cls, pred: List[List[str]], gt: List[List[str]]):
        assert len(pred) == len(gt)
        return best_sentence_level_bleu(gt, pred)
...
def best_sentence_level_bleu(ref_reports, hyp_reports):
    ...
    hun_macro = MacroAverage()
    hun_weighted = WeightedAverage()

    cham_macro = defaultdict(MacroAverage)
    cham_weighted = defaultdict(WeightedAverage)
    ...
    for i in range(len(ref_reports)):
        ...
        # hungarian
        bleu, row, col = best_report_bleu(ref_report, hyp_report, scores)
        hun_macro.add(bleu)
        hun_weighted.add(bleu)
        # chamfer
        recall, precision = best_report_bleu_chamfer(ref_report, hyp_report,
                                                     scores)
        cham_macro['recall'].add(list(recall))
        cham_macro['precision'].add(list(precision))
        cham_macro['f1'].add([f1_score(recall.mean(), precision.mean())])
        cham_weighted['recall'].add(list(recall))
        cham_weighted['precision'].add(list(precision))

    cham_macro_recall = cham_macro['recall'].val()
    cham_macro_precision = cham_macro['precision'].val()
    cham_macro_f1 = cham_macro['f1'].val()
    cham_micro_recall = cham_weighted['recall'].val()
    cham_micro_precision = cham_weighted['precision'].val()
    cham_micro_f1 = f1_score(cham_micro_precision, cham_micro_recall)

    return {
        'bleu4_macro': hun_macro.val(),
        'bleu4_weighted': hun_weighted.val(),
        'bleu4_chamfer_macro_f1': cham_macro_f1,
        'bleu4_chamfer_macro_recall': cham_macro_recall,
        'bleu4_chamfer_macro_precision': cham_macro_precision,
        'bleu4_chamfer_micro_f1': cham_micro_f1,
        'bleu4_chamfer_micro_recall': cham_micro_recall,
        'bleu4_chamfer_micro_precision': cham_micro_precision,
    }
...
def best_report_bleu(references, predicts, scores=None):
