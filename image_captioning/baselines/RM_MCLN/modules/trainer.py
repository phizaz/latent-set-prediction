...
class BaseTrainer(object):
    ...
    def eval(self):
        logs, results = self._eval()
        print(logs)
        record_path = os.path.join(self.args.save_dir, 'gen_test.pkl')
    ...
    def _print_best_to_file(self):
        ...
        record_table = record_table.append(self.best_recorder['val'],
                                           ignore_index=True)
        # record_table = record_table.append(self.best_recorder['test'],
        #                                    ignore_index=True)
...
class Trainer(BaseTrainer):
    def __init__(self, model, criterion, metric_ftns, optimizer, args,
                 ...
                 test_dataloader):
        super(Trainer, self).__init__(model, criterion, metric_ftns, optimizer,
                                      args)
    ...
    def _eval(self):
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            Image_id = []
            for batch_idx, (images_id, images, reports_ids,
                            reports_masks) in tqdm(
                                enumerate(self.test_dataloader),
                                total=len(self.test_dataloader),
                                desc='generating'):
                images, reports_ids, reports_masks = images.to(
                    self.device), reports_ids.to(
    ...
    def _train_epoch(self, epoch):
        ...
        #     test_gts, test_res = [], []
        #     for batch_idx, (images_id, images, reports_ids,
        #                     reports_masks) in tqdm(enumerate(self.test_dataloader), total=len(self.test_dataloader)):
        #         images, reports_ids, reports_masks = images.to(
        #             self.device), reports_ids.to(
        #                 self.device), reports_masks.to(self.device)
        #         output = self.model(images, mode='sample')
